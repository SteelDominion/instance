...
local function runPutAwayAnimation(context)
    ...
    if (context:getMagExtentLevel() == 3) then
        context:runAnimation("put_away_drum", track, false, PLAY_ONCE_HOLD, put_away_time * 0.75)
    else
...
local function runInspectAnimation(context)
    ...
    if (isNoAmmo(context)) then
        if (context:getMagExtentLevel() == 3) then
            context:runAnimation("inspect_empty_drum", track, false, PLAY_ONCE_STOP, 0.2)
        else
            context:runAnimation("inspect_empty", track, false, PLAY_ONCE_STOP, 0.2)
        end
    else
        if (context:getMagExtentLevel() == 3) then
            context:runAnimation("inspect_drum", track, false, PLAY_ONCE_STOP, 0.2)
        else
...
function base_track_state.entry(this, context)
    if (context:getMagExtentLevel() == 3) then
        context:runAnimation("static_idle_drum", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, LOOP, 0)
    else
...
function bolt_caught_states.bolt_caught.entry(this, context)
    if (context:getMagExtentLevel() == 3) then
        context:runAnimation("static_bolt_caught_drum", context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), false, LOOP, 0)
    else
...
function main_track_states.start.transition(this, context, input)
    if (input == INPUT_DRAW) then
        ...
        if (context:getMagExtentLevel() == 3) then
            context:runAnimation("draw_drum", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
        else
