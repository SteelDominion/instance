...
local function runInspectAnimation(context)
    ...
    if (isNoAmmo(context)) then
        ...
        if(context:getAmmoCount() == 1)then
            context:runAnimation("inspect_1left", track, false, PLAY_ONCE_STOP, 0.2)
        else
...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_BOLT) then
        if (context:getAttachment("SCOPE") == "murasamet:史山代码懒得改了") then
            context:runAnimation("bolt_pu", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        else
...
function main_track_states.reload.update(this, context)
    if (main_track_states.reload.loaded_ammo > main_track_states.reload.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if(context:isHolding(track)) then
            if(main_track_states.reload.emptyload == 1 and context:getAttachment("SCOPE") ~= "murasamet:史山代码懒得改了") then
                if (main_track_states.reload.loaded_ammo ~= 10) then
                    context:runAnimation("reload_load_clip", track, false, PLAY_ONCE_HOLD, 0.2)
                    main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 10
                else
