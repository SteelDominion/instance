...
local function runInspectAnimation(context)
    ...
    if (context:getAmmoCount() <= 1) then
        ...
        if(context:getAmmoCount() == 2)then
            context:runAnimation("inspect_2left", track, false, PLAY_ONCE_STOP, 0.2)
        else
...
function base_track_state.entry(this, context)
    if (context:getAttachment("SCOPE") == "murasamet:史山代码懒得改了") then
        --三个if让你彻底爱上这座史山！！！！！  ———— 可汗大滴 2026.4.11  4:20
...
function bolt_caught_states.bolt_caught.entry(this, context)
    if (context:getAttachment("SCOPE") == "murasamet:史山代码懒得改了") then
        --作者小提示：史山越多，代码越稳可靠！这是真的，原本这段加兰德有切枪后空挂叠加的bug，特地从几乎是一模一样但是很多史山的五六半状态机复制了这部分代码过来意外的发现居然修好了
        context:runAnimation("static_bolt_caught_pu", context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), false, LOOP, 0)
    else
        context:runAnimation("static_bolt_caught", context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), false, LOOP, 0)
    end--这个行为本身就是对欧姆尼塞亚的亵渎   —— —— 辉月.lua
end
...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_BOLT) then
        if (context:getAttachment("MAGAZINE") == "murasamet:garand_clips") then
...
function main_track_states.reload.entry(this, context)
    if (context:getAmmoCount() == 0) then
        ...
        if (context:getAmmoCount() == 1) then
            context:runAnimation("reload_start_no_other_ammo", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
        else
...
function main_track_states.reload.update(this, context)
    if (main_track_states.reload.loaded_ammo > main_track_states.reload.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if(context:isHolding(track)) then
            if(main_track_states.reload.emptyload == 1 and context:getAttachment("MAGAZINE") ~= "murasamet:garand_clips") then
                if (main_track_states.reload.loaded_ammo ~= 8) then
                    if (math.random(100) >= 96) then
                        --这里可以修改被咬手指的概率
                        context:runAnimation("reload_load_thumb_pinch", track, false, PLAY_ONCE_HOLD, 0.2)
                    else
                        context:runAnimation("reload_load_clip", track, false, PLAY_ONCE_HOLD, 0.2)
                    end
                else
                    if (context:getAmmoCount() >= 7) then
                        context:runAnimation("reload_loop_lastone", track, false, PLAY_ONCE_HOLD, 0.2)
                    else
...
function main_track_states.reload.transition(this, context, input)
    if (input == main_track_states.reload.retreat or input == INPUT_CANCEL_RELOAD) then
        if(main_track_states.reload.emptyload == 1 and context:getAttachment("MAGAZINE") ~= "murasamet:garand_clips") then
            ...
            context:runAnimation("reload_loop_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        end
        return main_track_states.idle
    end
