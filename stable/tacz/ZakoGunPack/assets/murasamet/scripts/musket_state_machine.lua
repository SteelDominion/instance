...
local function runReloadAnimation(context)
    ...
    if (isNoAmmo(context)) then
        ...
        elseif (ext == 2) then
            ...
            context:runAnimation("reload_empty_3", track, false, PLAY_ONCE_STOP, 0.2)
        end
    else
        if (ext == 0) then
            context:runAnimation("hey_man", track, false, PLAY_ONCE_STOP, 0.2)
        elseif (ext == 1) then
            context:runAnimation("there_is", track, false, PLAY_ONCE_STOP, 0.2)
        elseif (ext == 2) then
            context:runAnimation("nothing", track, false, PLAY_ONCE_STOP, 0.2)
        else
            context:runAnimation("to_see", track, false, PLAY_ONCE_STOP, 0.2)
        end
    end
end
