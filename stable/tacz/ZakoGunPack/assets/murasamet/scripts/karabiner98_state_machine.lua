...
local function runReloadAnimation(context)
    ...
    if (isNoAmmo(context)) then
        ...
        if(context:getAmmoCount() >= 1) then
            ...
            context:runAnimation("reload_start_1left", track, false, PLAY_ONCE_HOLD, 0)
        end
    end
end
...
function main_track_states.reload.entry(this, context)
    if (isNoAmmo(context)) then
        ...
        if (context:getAttachment("SCOPE") ~= "tacz:empty") then
            context:runAnimation("reload_start_scope_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
        else
    ...
    elseif(context:getAmmoCount() == 0) then
        if (context:getAttachment("SCOPE") ~= "tacz:empty") then
            context:runAnimation("reload_start_scope", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
        else
            if(context:getAmmoCount() >= 1) then
                context:runAnimation("reload_start", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
            else
                context:runAnimation("reload_start_1left", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
            end
        end
    else
        if (context:getAttachment("SCOPE") ~= "tacz:empty") then
            ...
            if(main_track_states.reload.emptyload == 1 and context:getAttachment("SCOPE") == "tacz:empty") then
                ...
                if (context:getAttachment("SCOPE") ~= "tacz:empty") then
                    if (context:getAmmoCount() == 0) and (context:hasBulletInBarrel() == false) then
                        context:runAnimation("reload_loop_scope_empty", track, false, PLAY_ONCE_HOLD, 0.2)
                    else
                        context:runAnimation("reload_loop_scope", track, false, PLAY_ONCE_HOLD, 0.2)
                    end
                else
                    context:runAnimation("reload_loop", track, false, PLAY_ONCE_HOLD, 0.2)
                end
...
function main_track_states.reload.transition(this, context, input)
    if (input == main_track_states.reload.retreat or input == INPUT_CANCEL_RELOAD) then
        if(main_track_states.reload.emptyload == 1 and context:getAttachment("SCOPE") == "tacz:empty") then
            ...
            if (context:getAttachment("SCOPE") ~= "tacz:empty") then
                context:runAnimation("reload_end_scope", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            else
