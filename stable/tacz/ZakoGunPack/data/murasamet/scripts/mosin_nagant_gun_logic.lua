...
function M.tick_reload(api)
    ...
    if (reloaded_count >= 0) then
        ...
        if(cache.needed_count == 5 and api:getAttachment("SCOPE") ~= "murasamet:scope_mosin_pu") then
            base_time = clip_load_feed
        end
        if (not cache.is_tactical) then
            base_time = base_time + (api:getAttachment("SCOPE") ~= "murasamet:scope_mosin_pu" and intro_empty_clip or intro_empty)
        else
